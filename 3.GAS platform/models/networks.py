...
class PatchSampleF(nn.Module):
    ...
    def create_mlp(self, feats):# 根据输入特征创建mlp, ([input_nc,nc],relu,[nc,nc])
        ...
        for feat_id, feat in enumerate(feats):
            ...
            if num_patches > 0:
                if patch_ids is not None:
                    ...
                    patch_id = patch_id[:int(min(num_patches, patch_id.shape[0]))]  # .to(patch_ids.device)#patch_id 决定的是坐标？所以取得是维度2的hxw这个维度，理解为在空间尺度上采样
                patch_id = torch.tensor(patch_id, dtype=torch.long, device=feat.device)
                x_sample = feat_reshape[:, patch_id, :].flatten(0, 1)  # reshape(-1, x.shape[1])
            else:
                x_sample = feat_reshape #全采
                patch_id = []

            # projection head + norm
            if self.use_mlp:
...
class ResnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect', no_antialias=False, no_antialias_up=False, opt=None):
        ...
        self.down1_conv = nn.Conv2d(64, 128, kernel_size=3, stride=2, padding=1, bias=use_bias)
        self.down1_norm = norm_layer(128)
        self.down1_ReLU = nn.ReLU(128)
        #self.down1_down = Downsample(128)
        
        
        self.down2_conv = nn.Conv2d(128, 256, kernel_size=3, stride=2, padding=1, bias=use_bias)
        self.down2_norm = norm_layer(256)
        self.down2_ReLU = nn.ReLU(256)
        #self.down2_down = Downsample(256)
        ...
        for i in range(n_blocks):       # add ResNet blocks
            ...
            model += [ResnetBlock(ngf * mult, padding_type=padding_type, norm_layer=AdaptiveInstanceNorm2d, use_dropout=use_dropout, use_bias=use_bias)]

        self.model = nn.Sequential(*model)

        #self.ASPP = ASPP() #6,12,18,24





        self.up1_up = Upsample(256)
        self.up1_conv = nn.Conv2d(256, int(128),kernel_size=3, stride=1,padding=1, bias=use_bias)
        self.up1_norm = norm_layer(128)
        self.up1_ReLU = nn.ReLU(True)


        self.up2_up = Upsample(128)
        self.up2_conv = nn.Conv2d(128, int(64),kernel_size=3, stride=1,padding=1, bias=use_bias)
        self.up2_norm = norm_layer(64)
        self.up2_ReLU = nn.ReLU(True)


        final = []
        final += [nn.ReflectionPad2d(3)]
        final += [nn.Conv2d(ngf, output_nc, kernel_size=7, padding=0)]
        final += [nn.Tanh()]

        self.final = nn.Sequential(*final)

        self.map1 = nn.Conv2d(128, 128, kernel_size=1, stride=1, bias=use_bias)
        self.map2 = nn.Conv2d(64, 64, kernel_size=1, stride=1, bias=use_bias)
        ...
        print(model)
        #print(model[0],model[4],model[8],model[12],model[16]) # model[20] is the final resblock
        #print(model[20])
        ...
        # create clip model to generate text hidden feature, used to generate style

        clip_model, _, preprocess = open_clip.create_model_and_transforms('ViT-B-32',
                                                                     pretrained="D:\\切片程序部署材料\\部署材料\\flask_test\\open_clip_pytorch_model.bin")
        tokenizer = open_clip.get_tokenizer('ViT-B-32')
        clip_model = clip_model.cuda()
        clip_model.eval()

        ffpe_style_des = tokenizer("formalin-fixed paraffin embedded tissues")
        #fs_style_des = tokenizer("frozen sections")


        ffpe_style_des = ffpe_style_des.cuda()
        #fs_style_des = fs_style_des.cuda()

        ffpe_style_des = clip_model.encode_text(ffpe_style_des)
        #fs_style_des = clip_model.encode_text(fs_style_des)

        # compute text direction

        #text_direction = (ffpe_style_des - fs_style_des).mean(axis=0, keepdim=True)
        ...
        ffpe_style_des /= ffpe_style_des.norm(dim=-1, keepdim=True)
        self.style_des = torch.tensor(ffpe_style_des, device=ffpe_style_des.device)



        # MLP to get AdaIN parameters, generate for different layer

        self.mapping = MLP(512, 256*4, 256*2, 6, norm='none', activ='lrelu') # share from all layers #
        ...
        for m in model.modules():
            ...
            if m.__class__.__name__ == "AdaptiveInstanceNorm2d":
                ...
                if i == 0:
                    ...
                    std = adain_params[:, m.num_features:2*m.num_features] # if input without real B, all param /2

                    m.bias = mean.contiguous().view(-1)
                    m.weight = std.contiguous().view(-1)
                    i = 1
                if i == 1:
                    mean = adain_params[:, 2*m.num_features:3*m.num_features]

                    std = adain_params[:, 3*m.num_features:4*m.num_features] # if input without real B, all param /2
                ...
                mean = adain_params[:,(i) * m.num_features :(i+1) * m.num_features]

                std = adain_params[:, (i+1) * m.num_features:(i+2) * m.num_features] # if input without real B, all param /2
    ...
    def forward(self, input, layers=[], encode_only=False):
        ...
        if len(layers) > 0:
            ...
            '''
            init = self.model(input) # 64x256x256
                   
            down_1 = self.down1_conv(init) # 128x128x128
            down_1 = self.down1_norm(down_1) # 128x128x128
            down_1 = self.down1_ReLU(down_1) # 128x128x128

            
            
            down_2 = self.down2_conv(down_1) # 256x64x64
            down_2 = self.down2_norm(down_2) # 256x64x64
            down_2 = self.down2_ReLU(down_2) # 256x64x64

            
            neck = self.resblock(down_2) # 256x64x64
            '''
            ...
            # cumpute param
            adain_param = self.mapping(self.style_des)

            # updata param
            self.assign_adain_params(adain_param, self.model)
            ...
            up1 = self.up1_up(input) # 256x128x128
            up1 = self.up1_conv(up1) # 128x128x128
            up1 = self.up1_norm(up1) # 128x128x128
            up1 = self.up1_ReLU(up1) # 128x128x128


            up1_res = up1 + self.relu(self.map1(down_1))



            up2 = self.up2_up(up1) # 128x256x256
            up2 = self.up2_conv(up2)  # 64x256x256
            up2 = self.up2_norm(up2) # 64x256x256
            up2 =  self.up2_ReLU(up2) # 64x256x256


            up2_res = up2 + self.relu(self.map2(init))




            fake = self.final(up2_res) # 3x256x256
